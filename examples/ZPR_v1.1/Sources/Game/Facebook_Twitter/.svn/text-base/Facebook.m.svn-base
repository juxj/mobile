...
static NSString* twitterURL = @"https://twitter.com/home?status=";
...
-(void)twitterDialog:(NSString *)action
		   andParams:(NSMutableDictionary *)params
		 andDelegate:(id <FBDialogDelegate>)delegate {
	
	[_fbDialog release];
	
	NSString *dialogURL = [twitterURL stringByAppendingString:action];
	_fbDialog = [[FBDialog alloc] initWithURL:dialogURL params:params delegate:delegate];
	[_fbDialog showTwitter];
}
