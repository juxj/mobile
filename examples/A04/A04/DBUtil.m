...
@synthesize delegate, dbFilePath;
...
-(id) initWithDBName:(NSString *) databaseName {
    ...
    if (self = [super init]) {
        self.dbFilePath = [[[NSBundle mainBundle] resourcePath ]stringByAppendingPathComponent:databaseName];
...
-(NSMutableArray *) getAll: (int) currPage {
    ...
    int startIndex = [self getStartIndex:currPage];
    sql = [NSString stringWithFormat:@"%@ %@ %d%@%d",
           sql, @"limit", startIndex, @",", APP_PAGE_SIZE];
    NSLog(@"%@", sql);
    return [self query:sql :currPage];
}

-(int) getStartIndex: (int) currPage {
    int startIndex = 0;
    if (currPage >  1){
        startIndex = (currPage-1)  * APP_PAGE_SIZE;
    }
    return startIndex;
}
